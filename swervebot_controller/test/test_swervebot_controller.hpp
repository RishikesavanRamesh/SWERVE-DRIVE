...
#ifndef TEMPLATES__ROS2_CONTROL__CONTROLLER__TEST_SWERVEBOT_CONTROLLER_HPP_
#define TEMPLATES__ROS2_CONTROL__CONTROLLER__TEST_SWERVEBOT_CONTROLLER_HPP_
...
#include "swervebot_controller/swervebot_controller.hpp"
...
using ControllerStateMsg = swervebot_controller::SwervebotController::ControllerStateMsg;
using ControllerReferenceMsg = swervebot_controller::SwervebotController::ControllerReferenceMsg;
using ControllerModeSrvType = swervebot_controller::SwervebotController::ControllerModeSrvType;
...
class TestableSwervebotController : public swervebot_controller::SwervebotController
{
  FRIEND_TEST(SwervebotControllerTest, all_parameters_set_configure_success);
  FRIEND_TEST(SwervebotControllerTest, activate_success);
  FRIEND_TEST(SwervebotControllerTest, reactivate_success);
  FRIEND_TEST(SwervebotControllerTest, test_setting_slow_mode_service);
  FRIEND_TEST(SwervebotControllerTest, test_update_logic_fast);
  FRIEND_TEST(SwervebotControllerTest, test_update_logic_slow);

public:
  controller_interface::CallbackReturn on_configure(
    const rclcpp_lifecycle::State & previous_state) override
  {
    auto ret = swervebot_controller::SwervebotController::on_configure(previous_state);
...
class SwervebotControllerFixture : public ::testing::Test
{
public:
  ...
  void SetUp()
  {
    ...
    command_publisher_ = command_publisher_node_->create_publisher<ControllerReferenceMsg>(
      "/test_swervebot_controller/commands", rclcpp::SystemDefaultsQoS());
    ...
    slow_control_service_client_ = service_caller_node_->create_client<ControllerModeSrvType>(
      "/test_swervebot_controller/set_slow_control_mode");
  }
...
protected:
  void SetUpController(const std::string controller_name = "test_swervebot_controller")
  {
  ...
  void subscribe_and_get_messages(ControllerStateMsg & msg)
  {
    ...
    auto subscription = test_subscription_node.create_subscription<ControllerStateMsg>(
      "/test_swervebot_controller/state", 10, subs_callback);
...
protected:
  ...
  std::unique_ptr<TestableSwervebotController> controller_;
...
#endif  // TEMPLATES__ROS2_CONTROL__CONTROLLER__TEST_SWERVEBOT_CONTROLLER_HPP_
